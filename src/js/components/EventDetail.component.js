...
export default class EventDetail extends React.Component
{
        ...
        return <div className="prop-section basic-props">
                ...
                <div className="name">
                    <button onClick={this.handleCourseClick.bind(this, this.props.data.course)}>
                        ...
                        this.props.data.name : this.getLinkName('courses', this.props.data.course)}
                    </button>
                </div>
        ...
        if ( this.props.data.details.appliedExceptions && this.props.data.details.appliedExceptions.length > 0 )
        {
            return <div className="prop-section exceptions">
                    <button className="reveal" onClick={this.revealHidden.bind(this)}>
                        ...
                        {CP.translate( 'detail.cancelled' )}
                    </button>
                    <div className="hideable hide">
                        <div className="because">{CP.translate( 'detail.becauseof' )}</div>
                        {this.props.data.details.appliedExceptions.map( function( exception ) {
                                return <div><strong>{this.getLinkName('exceptions', exception)}</strong></div>;
                            }.bind( this ) )}
                    </div>
                </div>;
        }
    ...
    eventNumericProps()
    {
        return <div className="prop-section num-props">
                {this.numPropertyField( CP.translate( 'detail.students' ),
                    this.props.data.details.students ? this.props.data.details.students.length : "?" )}

                {this.props.data.details.capacity ?
                    this.numPropertyField( CP.translate( 'detail.capacity' ), this.props.data.details.capacity, 'center' ) :
                    <div />}

                {this.props.data.details.parallel ?
                    this.numPropertyField( CP.translate( 'detail.parallel' ), CP.translate( 'detail.number', {num: this.props.data.details.parallel } ), 'right' ) :
                    <div />}
            </div>;
    }
    ...
    eventTeachers()
    {
        return <div className="prop-section teachers">
                ...
                <div className="prop-title">{CP.translate( 'detail.teachers' )}</div>
            </div>;
    }
    ...
    teacherField( username, fullname )
    {
        return <div className="teacher">
                <button className="reveal" onClick={this.revealHidden.bind(this)}>
                    <i className="fa fa-male"></i><i className="fa fa-caret-down caret"></i>
                    {fullname}
                </button>
                <div className="hideable hide">
                    <a href={'https://usermap.cvut.cz/profile/' + username}>{CP.translate('detail.usermap_profile')}</a><br />
                    <button onClick={this.handleTeacherClick.bind(this, username)}>{CP.translate('detail.personal_calendar')}</button>
                </div>
            </div>;
    }
