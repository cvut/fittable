  ...
  render () {
    ...
    for (var i = Math.ceil(this.props.grid.starts); i < ( this.props.grid.facultyGrid ? Math.ceil(this.props.grid.starts) + this.props.grid.facultyHours : this.props.grid.ends); i++) {
      hourlabels.push(<div
                           ...
                           key={idx}
                           style={{ width: timelineGridLength * 100 + '%', height: timelineGridLength * 100 + '%', left: ( idx + gridoffset) * timelineGridLength * 100 + '%', top: ( idx + gridoffset) * timelineGridLength * 100 + '%' }}>
    ...
    return <div className={ 'table ' + (this.state.popupsOpened> 0 ? 'muted ' : '') + this.props.layout + ( this.props.functionsOpened !== null ? ' cut' : '') + ( this.props.days7 ? ' days7' : '')} ref="rootEl">
             <div className="grid-overlay">
               <div className="grid-wrapper">
                 <div className="grid hor" style={{ backgroundSize: ( timelineGridLength * 100) + '% 100%', backgroundPosition: ( gridoffset * -100) + '% 0%' }}></div>
                 <div className="grid ver" style={{ backgroundSize: '100% ' + ( timelineGridLength * 100) + '%', backgroundPosition: '0% ' + ( gridoffset * -100) + '%' }}></div>
               </div>
             </div>
             <NowIndicator
